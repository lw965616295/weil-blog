$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '预览图', name: 'coverImage', index: 'cover_image', width: 120, formatter: coverImageFormatter},
            {label: '浏览量', name: 'views', index: 'views', width: 60},
            {label: '状态', name: 'status', index: 'status', width: 60, formatter: statusFormatter},
            {label: '博客分类', name: 'categoryName', index: 'category_name', width: 60},
            {label: '添加时间', name: 'createDate', index: 'create_date', width: 90}
        ],
        height: 700,
        rowNum: 10,
        rowList: [10, 20, 50],
...
function editBlog() {
    ...
    window.location.href = "/admin/blog/detail?id=" + id;
}
