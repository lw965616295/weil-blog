$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '评论内容', name: 'comment', index: 'commentBody', width: 120},
            {label: '评论时间', name: 'createDate', index: 'commentCreateTime', width: 60},
            {label: '评论人名称', name: 'commentator', index: 'commentator', width: 60},
            {label: '评论人邮箱', name: 'email', index: 'email', width: 90},
            {label: '状态', name: 'status', index: 'commentStatus', width: 60, formatter: statusFormatter},
            {label: '回复内容', name: 'replyContent', index: 'replyBody', width: 120},
        ],
        height: 700,
        rowNum: 10,
        rowList: [10, 20, 50],
...
function reply() {
    ...
    console.log(rowData);
    if (rowData.status.indexOf('待审核') > -1) {
        swal("请先审核该评论再进行回复!", {
            icon: "warning",
        });
        return;
...
$('#saveButton').click(function () {
    ...
    // } else {
        ...
        var id = getSelectedRow();
        var params = {"id": id, "replyContent": replyBody}
        $.ajax({
            type: 'POST',//方法类型
            url: url,
            contentType: "application/json",
